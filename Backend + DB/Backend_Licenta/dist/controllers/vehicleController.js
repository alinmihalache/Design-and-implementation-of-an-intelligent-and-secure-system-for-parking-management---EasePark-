...
exports.deleteVehicle = exports.addVehicle = exports.getUserVehicles = void 0;
const database_1 = __importDefault(require("../config/database"));
const getUserVehicles = async (req, res) => {
    try {
        const userId = parseInt(req.params.userId);
        const result = await database_1.default.query('SELECT * FROM vehicles WHERE user_id = $1', [userId]);
...
const addVehicle = async (req, res) => {
    try {
        ...
        const plateExists = await database_1.default.query('SELECT id FROM vehicles WHERE plate_number = $1', [plateNumber]);
        if (plateExists.rows.length > 0) {
            res.status(400).json({ message: 'Vehicle with this plate number already exists' });
            return;
        }
        const result = await database_1.default.query('INSERT INTO vehicles (user_id, plate_number, make, model, year, type) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [userId, plateNumber, make, model, year, type]);
...
const deleteVehicle = async (req, res) => {
    try {
        ...
        const vehicle = await database_1.default.query('SELECT * FROM vehicles WHERE id = $1 AND user_id = $2', [vehicleId, userId]);
        if (vehicle.rows.length === 0) {
            res.status(404).json({ message: 'Vehicle not found or unauthorized' });
            return;
        }
        // Check if vehicle has active reservations
        const activeReservations = await database_1.default.query(`SELECT id FROM reservations 
       WHERE vehicle_id = $1 
       AND status IN ('pending', 'active')`, [vehicleId]);
        if (activeReservations.rows.length > 0) {
            res.status(400).json({ message: 'Cannot delete vehicle with active reservations' });
            return;
        }
        const result = await database_1.default.query('DELETE FROM vehicles WHERE id = $1 RETURNING *', [vehicleId]);
