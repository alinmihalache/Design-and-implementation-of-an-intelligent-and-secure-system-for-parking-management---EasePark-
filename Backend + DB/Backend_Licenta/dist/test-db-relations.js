...
async function testRelations() {
    try {
        console.log('--- USERS ---');
        const users = await database_1.default.query('SELECT id, username, email, role FROM users');
        console.table(users.rows);
        console.log('\n--- VEHICLES ---');
        const vehicles = await database_1.default.query('SELECT id, user_id, plate_number, make, model, year, type FROM vehicles');
        console.table(vehicles.rows);
        console.log('\n--- PARKING SPOTS ---');
        const spots = await database_1.default.query('SELECT id, latitude, longitude, address, price_per_hour, type FROM parking_spots');
        console.table(spots.rows);
        console.log('\n--- RESERVATIONS (with user & vehicle) ---');
        const reservations = await database_1.default.query(`
    ...
    `);
        ...
        const payments = await database_1.default.query(`
